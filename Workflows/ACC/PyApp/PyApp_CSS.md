---
WorkflowId: PyApp_CSS
Type: Workflow_Plural
Domain: PyApp
Scope: Generate PyApp App-Level CSS (planning + code + docs) via child workflows
Status: Draft
Version: 26.00.00
...
Example:
- Workflow( "PyApp_CSS", "E-Commerce, Trip", "Blue brand + premium feel; reusable layout utilities" )
...
This workflow creates/updates the **single global PyApp design-system CSS**:
- `{RoleRoot}/css/0.css`
...
Hard rule:
- If app/theme root is missing => error `WF_APPROOT_NOT_FOUND` (run `PyApp_Structure` first).
...
Call:
- Workflow( "PyApp_CSS-Planning", "{AppName}", "{Description}" )

## Step C: Code Generation (Child Workflow)
Call:
- Workflow( "PyApp_CSS-Code", "{AppName}", "{Description}" )

## Step D: Documentation (Child Workflow)
Call:
- Workflow( "PyApp_CSS-Documentation", "{AppName}", "{Description}" )
