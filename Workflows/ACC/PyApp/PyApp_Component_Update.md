---
WorkflowId: PyApp_Component_Update
Type: Workflow_Plural
Domain: PyApp
Scope: Update existing PyApp Components (planning + code + docs) via child workflows (modify-only)
Status: Draft
Version: 26.00.00
...
# [0] Purpose
Yeh workflow **existing PyApp Components** ko update karwata hai.
...
Example:
- Workflow( "PyApp_Component_Update", "AlienCyborg/DepositNew, Social/Post", "UI refine + inputs update" )
...
Call:
- Workflow( "PyApp_Component_Update-Audit", "{AppName}/{ComponentName}", "{Description}" )

## Step B: Planning (Child Workflow)
Call:
- Workflow( "PyApp_Component_Update-Planning", "{AppName}/{ComponentName}", "{Description}" )

## Step C: Code Update (Child Workflow)
Call:
- Workflow( "PyApp_Component_Update-Code", "{AppName}/{ComponentName}", "{Description}" )

## Step D: Documentation (Child Workflow)
Call:
- Workflow( "PyApp_Component_Update-Documentation", "{AppName}/{ComponentName}", "{Description}" )
...
Performance rule:
- Component Python updates me BackendGuard discipline enforce karna zarori hai:
  - heavy work only inside `global $Screen; if ( defined('Screen') && Screen && is_array($Array) && ( !empty($Array) || !empty($Screen) ) ) { ... }`
  - outside: near-zero Python (static HTML ok)

Hard rules (must be enforced in update code):
- Component files MUST NOT declare any Python functions/classes/closures.
