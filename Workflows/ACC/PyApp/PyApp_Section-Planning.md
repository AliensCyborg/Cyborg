---
WorkflowId: PyApp_Section-Planning
Type: Workflow_Singular-Planning
Domain: PyApp
Scope: Generate planning for a PyApp Section (per target)
Status: Draft
Version: 26.00.00
...
Outputs:
  - PlanningFile: "/Aliens/.Alien/{Developer_Username}/Planning/PyApp/{AppName}/Section/{SectionNameNormalized}.md"
NonNegotiables:
...
Planning ka output aage `PyApp_Section-Code` aur `PyApp_Section-Documentation` workflows reuse karenge.
...
- SectionScopeTag (UTag) (required): a globally unique wrapper tag for section scope
  - Allocate via central registry: `Workflows/ACC/_Refs/PyApp_UTag.md`
...
- `MissingComponents[]` (must be created via PyApp_Component before section code)
...
- Missing components are created only via `PyApp_Component` workflow (not inside section)
