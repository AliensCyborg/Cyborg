---
WorkflowId: PyApp_Update-Planner
WorkflowName: PyApp Update Planner
...
# [0] Purpose
`Workflow("PyApp_Update-Planner", "{AppName}", "{Description}")`

Convert `PyApp_Update-Planning` into an executable CSV planner (task list) for updates.
...
- Update planner schema MUST remain a strict superset-compatible match with the main `PyApp-Planner` schema.
...
Derivation SSOT:
- Use the same deterministic mapping defined in `PyApp-Planner` for base `Code_Path/Content_Path`.
...
- `Worklflow` MUST reference existing ACC workflow ids, usually from:
  - `PyApp_Screen_Update`
  - `PyApp_Section_Update`
  - `PyApp_Component_Update`
...
- `ID>=1` rows must have enterprise-grade prompt in `Description` (see main PyApp planner prompt contract).

---

# [4.1] `Description` Prompt Contract (Update Planner)
Same contract as `PyApp-Planner`:
