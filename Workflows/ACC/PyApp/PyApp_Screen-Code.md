---
WorkflowId: PyApp_Screen-Code
Type: Workflow_Singular-Code
Domain: PyApp
Scope: Generate a PyApp Screen + its Screen CSS file (create-only)
Status: Draft
Version: 26.00.00
...
- If ScreenOut exists OR CssOut exists:
  ...
  - Action: run `PyApp_Screen_Update` workflow
...
- Action: run `PyApp_Screen-Planning`
...
## 5.2 Required Screen Python Patterns
...
- Agar UI me new section/component chahiye ho, to separate rows/workflows use hon (`PyApp_Section`, `PyApp_Component`) and screen rerun ho.

Dependency gate (mandatory):
  - `{AppRoot}/User/section/{SectionNameNormalized}.py` exists
  ...
  - Action: run `PyApp_Section` for missing sections (via plural orchestrator), then re-run `PyApp_Screen`.
