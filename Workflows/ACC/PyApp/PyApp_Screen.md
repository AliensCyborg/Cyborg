---
WorkflowId: PyApp_Screen
Type: Workflow_Plural
Domain: PyApp
Scope: Generate PyApp Screens (planning + code + docs) via child workflows
Status: Draft
Version: 26.00.00
...
# [0] Purpose
Yeh workflow **1 ya multiple PyApp Screens** generate karwata hai.
...
Example:
- Workflow( "PyApp_Screen", "AlienCyborg/Deposit, AlienCyborg/Home", "UI premium hona zarori hai" )
...
- Resolve paths deterministically:
  ...
  - ScreenOut: `{AppRoot}/User/screen/{ScreenNameNormalized}.py`
...
Call:
- Workflow( "PyApp_Screen-Planning", "{AppName}/{ScreenName}", "{Description}" )
...
- For each required section:
  ...
  - If missing: pehle `PyApp_Section` workflow run karo (loop / per section), phir screen me call.
...
- Action: run `PyApp_Section` for missing sections, then re-run screen.

## Step C: Code Generation (Child Workflow)
Call:
- Workflow( "PyApp_Screen-Code", "{AppName}/{ScreenName}", "{Description}" )

## Step D: Documentation (Child Workflow)
Call:
- Workflow( "PyApp_Screen-Documentation", "{AppName}/{ScreenName}", "{Description}" )
...
- Allowed dependency creation (no extra pause/approval prompts):
  - Missing Sections ko `PyApp_Section` workflow se auto-create karna (same theme), phir screen me call karna.
  - If those Sections Planning contract defines `RequiredComponents[]`, then `PyApp_Section` dependency resolver may auto-run `PyApp_Component` to create missing components (same theme) — **without stopping for separate approval**.
