---
WorkflowId: PyApp_Component-Planning
Type: Workflow_Singular-Planning
Domain: PyApp
Scope: Generate planning for a PyApp Component (per target)
Status: Draft
Version: 26.00.00
...
Outputs:
  - PlanningFile: "/Aliens/.Alien/{Developer_Username}/Planning/PyApp/{AppName}/Component/{ComponentName}.md"
NonNegotiables:
...
Planning ka output aage `PyApp_Component-Code` aur `PyApp_Component-Documentation` workflows reuse karenge.
...
- UTag format is SSOT-driven (do not guess): `Workflows/ACC/_Refs/PyApp_UTag.md`
...
- Component UTag must be globally unique across the entire PyOS ecosystem (all PyApp).
- Manual pick forbidden. Allocate via central registry:
  - SSOT: `Workflows/ACC/_Refs/PyApp_UTag.md`

Planning must include the allocated UTag.

## Step E: Define PyOS Screen Contract (Required)
...
1) `{AppRoot}/component/{ComponentName}.py`
2) `{AppRoot}/css/{ComponentName}.css`
No more.

## Step G: Premium UI Acceptance (Minimal Checklist)
- AliensStyle header/footer present in Python component
...
- Component Python contains no `function` declarations.
