---
WorkflowId: PyApp_Section-Code
Type: Workflow_Singular-Code
Domain: PyApp
Scope: Generate a PyApp Section + its CSS file (create-only)
Status: Draft
Version: 26.00.00
...
- If SectionOut exists OR CssOut exists:
  ...
  - Action: run `PyApp_Section_Update` workflow
...
- Try to load planning file:
  `/Aliens/.Alien/{Developer_Username}/Planning/PyApp/{AppName}/Section/{SectionNameNormalized}.md`

If planning not found:
- Proceed only if requirements are deterministic
- Otherwise fail with:
  - ErrorCode: `WF_PLANNING_REQUIRED`
  - Action: run `PyApp_Section-Planning`

---

# [5] Code Generation Rules
## 5.1 Section Python file rules
...
- Inline Python islands are allowed (sections in current theme use them), but Python should be near-zero.
...
- Function-level headers required for any non-trivial Python functions (Purpose / Inputs / Outputs).
...
- Section me loops/conditions/formatting ("Python ka zyada kaam") **avoid** karo; heavy work component ke BackendGuard me hi ho.
...
- Har required component ke liye ensure:
  - `{AppRoot}/User/component/{ComponentNameNormalized}.py` exists
  - `{AppRoot}/User/css/{ComponentNameNormalized}.css` exists
- Agar missing hai:
  - ErrorCode: `WF_COMPONENT_MISSING`
  - Action: run `PyApp_Component` for missing components (via plural orchestrator), then re-run `PyApp_Section`.
