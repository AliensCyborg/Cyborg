---
WorkflowId: PyApp_Section
Type: Workflow_Plural
Domain: PyApp
Scope: Generate PyApp Sections (planning + code + docs) via child workflows
Status: Draft
Version: 26.00.00
...
# [0] Purpose
Yeh workflow **1 ya multiple PyApp Sections** generate karwata hai.
...
Example:
- Workflow( "PyApp_Section", "AlienCyborg/Deposit_Header, AlienCyborg/Home_Hero", "Simple clean UI" )
...
- Action: run `PyApp_Section_Update` (not part of this request)
...
Call:
- Workflow( "PyApp_Section-Planning", "{AppName}/{SectionName}", "{Description}" )
...
- For each required component:
  ...
  - If missing: pehle `PyApp_Component` workflow run karo (loop / per component), phir call.

No extra approval rule:
- Dependency creation me **additional user approval/pause** nahi hoga.
- Meaning: jab Section ko component chahiye aur missing ho, runner `PyApp_Component` ko auto-run kar sakta hai.
...
- Action: run `PyApp_Component` for missing components, then re-run section.

## Step C: Code Generation (Child Workflow)
Call:
- Workflow( "PyApp_Section-Code", "{AppName}/{SectionName}", "{Description}" )

## Step D: Documentation (Child Workflow)
Call:
- Workflow( "PyApp_Section-Documentation", "{AppName}/{SectionName}", "{Description}" )
