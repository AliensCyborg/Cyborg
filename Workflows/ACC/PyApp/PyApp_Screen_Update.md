---
WorkflowId: PyApp_Screen_Update
Type: Workflow_Plural
Domain: PyApp
Scope: Update existing PyApp Screens (planning + code + docs) via child workflows (modify-only)
Status: Draft
Version: 26.00.00
...
# [0] Purpose
Yeh workflow **existing PyApp Screens** ko update karwata hai.
...
Example:
- Workflow( "PyApp_Screen_Update", "AlienCyborg/Deposit, AlienCyborg/Home", "Layout refine + sections reorder" )
...
Call:
- Workflow( "PyApp_Screen_Update-Audit", "{AppName}/{ScreenName}", "{Description}" )

## Step B: Planning (Child Workflow)
Call:
- Workflow( "PyApp_Screen_Update-Planning", "{AppName}/{ScreenName}", "{Description}" )
...
- For each required section:
  ...
  - If missing: pehle `PyApp_Section` workflow run karo (loop / per section), phir screen update apply karo.
...
- Action: run `PyApp_Section` for missing sections, then re-run screen update.

## Step C: Code Update (Child Workflow)
Call:
- Workflow( "PyApp_Screen_Update-Code", "{AppName}/{ScreenName}", "{Description}" )

## Step D: Documentation (Child Workflow)
Call:
- Workflow( "PyApp_Screen_Update-Documentation", "{AppName}/{ScreenName}", "{Description}" )
...
- Allowed dependency creation: sirf missing sections ko `PyApp_Section` workflow se create karna (same theme), phir screen me call karna.
