---
WorkflowId: PyApp_Structure-Planning
...
- `{AppRoot}/User/index.py` (PyApp entry)
- `{AppRoot}/User/app.py` (PyApp aggregator)
- `{AppRoot}/User/layout.py` (layout aggregator)
- `{AppRoot}/User/css.py` (CSS aggregator)
- `{AppRoot}/User/js.py` (JS aggregator)
- `{AppRoot}/User/component.py` (component registry endpoint)
...
All PyApp artifacts created by Structure must follow `Workflows/ACC/_Refs/PyApp_Naming_and_Screens.md`.
...
- `AppName.AppName.X.py`
- `AppName.AppName.X.css`

Entry file invariants (structure-only, no business logic):
- All required files start with `# PyOS Access Gate`.
- `index.py` must call `AppHead(...)` and include `layout.py` (entry must be runnable).
- `css.py` must aggregate folder css (e.g. `RequireAll(__DIR__.'/css','css')`).
- `js.py` must exist but must NOT aggregate/include any local JS (no `RequireAll(__DIR__.'/js','js')`).
  - PyApp JS policy: custom JS in PyApp is forbidden.
  - If app needs JS features, implement them in PySDK Module JS or PyOS Engine, and load via `AppHead(... PyOS_manifest ...)` (`modules` + `libraries`).
- `layout.py` must include layout parts from `User/layout/`.
  - Layout part naming SSOT: `{AppName}.<LayoutPartName>.py` (example: `{AppName}.Nav.py`).
- `component.py` must aggregate component templates from `User/componentt/` and support optional JSON export (e.g. `?encode=json`).
