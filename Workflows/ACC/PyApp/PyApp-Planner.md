---
WorkflowId: PyApp-Planner
WorkflowName: PyApp Planner
...
# [0] Purpose
`Workflow("PyApp-Planner", "{AppName}", "{Description}")`

Convert `PyApp-Planning` into an executable CSV planner (task list) similar to the sample `Workflows/Examples/Planner/B-Commerce.csv`.
...
- `utag` = **UTag HTML wrapper tag** (globally unique) for UI artifacts.
	...
	- Non-negotiable: PyOS ecosystem me UTag kabhi collide nahi karti (across all PyApp).
	- Allocation SSOT: `Workflows/ACC/_Refs/PyApp_UTag.md`
...
- `Name` meaning depends on `Worklflow` type:
	- For file-based PyApp UI workflows (`PyApp_Screen`, `PyApp_Section`, `PyApp_Component` + update variants), `Name` is the **filename basename seed** (STRICT).
		- It MUST follow normalization + prefix SSOT in `Workflows/ACC/_Refs/PyApp_Naming_and_Screens.md`.
...
- `Parent` + `Parent_Type` = hierarchy link. Example: Parent=`PyApp_Structure`, Parent_Type=`PyApp_Structure`.
...
- Resolve PyApp `{Version}` as the latest numeric folder under `/Aliens/PyApp/{AppName}/`.
- If listing directories is not possible, fallback `{Version}=26.00.00`.
- `{AppTheme}={AppName}_Basic`.

Base:
- `{AppRoot}=/Aliens/PyApp/{AppName}/{Version}/theme/{AppTheme}`
- Default role folder: `{AppRoot}/User/`.

### A) PyApp Structure
If `Worklflow=PyApp_Structure`:
- `Planning_Path` = `/Aliens/.Alien/{Developer}/Planning/PyApp/{AppName}/PyApp-Planning.md`
- `Code_Path` = `{AppRoot}/User/index.py`
- `Content_Path` = `{AppRoot}/User/css/0.css`
- `Doc_Path` = `/Aliens/Docs/PyApp/{AppName}/PyApp_Structure.md`

### B) PyApp Screen
If `Worklflow=PyApp_Screen`:
- `Planning_Path` = `/Aliens/.Alien/{Developer}/Planning/PyApp/{AppName}/Screen/{ScreenNameNormalized}.md`
- `Code_Path` = `{AppRoot}/User/screen/{ScreenNameNormalized}.py`
- `Content_Path` = `{AppRoot}/User/css/{ScreenNameNormalized}-Screen.css`
- `Doc_Path` = `/Aliens/Docs/PyApp/{AppName}/Screens/{ScreenNameNormalized}.md`

### C) PyApp Section
If `Worklflow=PyApp_Section`:
- `Planning_Path` = `/Aliens/.Alien/{Developer}/Planning/PyApp/{AppName}/Section/{SectionNameNormalized}.md`
- `Code_Path` = `{AppRoot}/User/section/{SectionNameNormalized}.py`
- `Content_Path` = `{AppRoot}/User/css/{SectionNameNormalized}-Section.css`
- `Doc_Path` = `/Aliens/Docs/PyApp/{AppName}/Sections/{SectionNameNormalized}.md`

### D) PyApp Component
If `Worklflow=PyApp_Component`:
- `Planning_Path` = `/Aliens/.Alien/{Developer}/Planning/PyApp/{AppName}/Component/{ComponentNameNormalized}.md`
- `Code_Path` = `{AppRoot}/User/component/{ComponentNameNormalized}.py`
- `Content_Path` = `{AppRoot}/User/css/{ComponentNameNormalized}.css`
- `Doc_Path` = `/Aliens/Docs/PyApp/{AppName}/Components/{ComponentNameNormalized}.md`

### E) PyApp Layout
If `Worklflow=PyApp_Layout`:
- `Planning_Path` = `/Aliens/.Alien/{Developer}/Planning/PyApp/{AppName}/Layout/{LayoutNameNormalized}.md`
- `Code_Path` = `{AppRoot}/User/layout/{LayoutNameNormalized}.py`
- `Content_Path` = `{AppRoot}/User/css/{LayoutNameNormalized}-Layout.css` (optional; if layout CSS is required)
- `Doc_Path` = `/Aliens/Docs/PyApp/{AppName}/Layouts/{LayoutNameNormalized}.md`

### F) PyApp Global CSS
If `Worklflow=PyApp_CSS` (or `PyApp_CSS_Update`):
- `Name` allowed:
	- Recommended: `{AppName}.Global`
	- Also allowed: `{AppName}` (runner derives AppName deterministically)
- `Planning_Path` = `/Aliens/.Alien/{Developer}/Planning/PyApp/{AppName}/CSS/{AppName}.Global.md`
- `Code_Path` = `{AppRoot}/User/css/0.css`
- `Content_Path` = blank (global CSS is the primary code artifact)
- `Doc_Path` = `/Aliens/Docs/PyApp/{AppName}/CSS/{AppName}.Global.md`

Hard rule:
- PyApp JS rows are forbidden; planner MUST use `Worklflow=PySDK_Module_JS` for JS behavior.
...
- `Worklflow` column must reference an existing ACC workflow id (examples: `PyApp_Screen`, `PyApp_Section`, `PySDK_Module_API`).
...
- `__PlannerMeta__` row ka `Description` **ultra-detailed** hona non-negotiable hai. Is me clear define karo:
	- Is planner se exactly kya kya build hoga (PyApp + PySDK scope)
...
- `PyApp_Layout`, `PyApp_Screen`, `PyApp_Section`, `PyApp_Component` workflows har unit ki **apni CSS** file generate karte hain (jitne units utni CSS). Is liye in rows ke `Description` me UI ko ultra-detail me describe karna mandatory hai.
- Exactly **ONE** global CSS row allowed:
	- Recommended: `ID=2` with `Worklflow=PyApp_CSS` (GLOBAL ONLY).
	- This row MUST generate/update `{AppRoot}/User/css/0.css`.
	- Additional `PyApp_CSS` rows (per-screen/per-section/per-component) are forbidden.
...
- Planner me reusable `PyApp_Component` rows zaroor add karo (shared controls, cards, headers, list-items, empty-states, skeletons).
- Baaki components/sections/screens me duplication forbidden; reuse via `Component()` calls.

Assets rule (NON-NEGOTIABLE):
- Images/icons/fonts/any assets sirf `PyApp_Component` rows ke `Description` me define karo.
- Asset acquisition (create/download/place) `PyApp_Component` workflow handle karega; descriptions me deterministic source + expected local target paths + caching/bundling rules specify karo.

Enterprise baseline coverage (planner should include these categories unless scope explicitly excludes):
- `PyApp_Structure` row for scaffolding (or update structure when running update scenario).
...
PyApp JS placement rule (non-negotiable):
- PyApp ke andar custom JavaScript implement karna forbidden hai.
- If the app needs JS features, planner MUST create a module-level JS task row using `Worklflow=PySDK_Module_JS`.
- Default module for PyApp JS is `User` unless the requirement explicitly states a different module.

UTag non-negotiable rule (ecosystem-level):
- Screen/Section/Component rows ke liye `utag` MUST be globally unique.
- Multi-PC safety ke liye manual UTag pick forbidden. Allocation MUST follow `Workflows/ACC/_Refs/PyApp_UTag.md`.
...
PyApp UI rows additional mandatory rule:
- If `Worklflow` is `PyApp_Layout` / `PyApp_Screen` / `PyApp_Section` / `PyApp_Component` then `[UI]` section is NOT optional — it is mandatory and must include:
...
PyApp Component rows additional mandatory rule:
...
- Parent chain contract is strict: Component -> Section -> Screen -> PyApp (no cross-parenting).
...
- PyApp actual me multiple screens ko host karta hai (routing/wiring), UI hierarchy ko break nahi karta.
...
## [4.3.0] PyApp Global CSS row (example template)
Use when `Worklflow=PyApp_CSS` (GLOBAL ONLY):
...
Use when `Worklflow=PyApp_Screen` / `PyApp_Screen-Code`:

"[OBJECTIVE]
Create a complete, production-ready PyApp Screen for {AppName}, deeply integrated with PyOS (not standalone).
...
[DATA]
- Define required fields and data flow; prefer existing PyOS engines/modules.
...
## [4.3.2] PySDK API row (example template)
Use when `Worklflow=PySDK_Module_API` (or similar):

"[OBJECTIVE]
Implement an enterprise-grade API/module unit for PyOS/PySDK integration.
...
- Use PyOS routing/config/logging/hooks/lifecycle where possible.
...
## [4.3.3] PySDK Module JS row (example template)
Use when `Worklflow=PySDK_Module_JS` (module-level JS; preferred for PyApp JS needs):

"[OBJECTIVE]
Implement enterprise-grade module JS for PyApp needs (no PyApp-local JS).
...
[OUTPUT CONTRACT]
- Module Python entry (must exist / be verified): /Aliens/PySDK/Py.26.00.00/Module/{Name}/py/index.py
- Primary JS: {Code_Path} (must be /Aliens/PySDK/Py.26.00.00/Module/{Name}/js/index.js)
- Docs: {Doc_Path}
...
- No PyApp files contain custom JS logic.
...
"[OBJECTIVE]
Generate COMPLETE, PRODUCTION-READY, ULTRA ENTERPRISE-GRADE output for this Planner row, deeply integrated with PyOS ecosystem (not standalone).
...
- Prefer existing PyOS engines/modules/routing/config/logging/hooks/lifecycle where possible.
...
[ACCEPTANCE]
- No Python warnings/errors (where applicable).
