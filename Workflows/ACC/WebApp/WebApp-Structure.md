---
WorkflowId: WebApp-Structure
WorkflowName: WebApp Structure
...
# [0] Purpose
`Workflow("WebApp-Structure", "{AppName}", "{Description}")`
...
## [3.1] Canonical WebApp Root (SSOT)
...
Rules:
- `Code_Path` MUST point to the primary code artifact for that task (PHP/JS/etc.) when applicable.
...
## [3.3] Deterministic Mapping (WebApp Screen/Section/Component)
Derive `{NameNormalized}` using the same normalization rules defined in `Workflows/ACC/_Refs/WebApp_Naming_and_Screens.md`.

When `Worklflow` references one of these groups, use the mappings below.

### Screen group
Worklflow in:
- `WebApp_Screen`, `WebApp_Screen-Code`, `WebApp_Screen_Update`, `WebApp_Screen_Update-Code`

Create empty placeholders:
- `{AppRoot}/User/screen/{NameNormalized}.php`
- `{AppRoot}/User/css/{NameNormalized}-Screen.css`

Set CSV columns:
- `Code_Path = {AppRoot}/User/screen/{NameNormalized}.php`
...
### Section group
Worklflow in:
- `WebApp_Section`, `WebApp_Section-Code`, `WebApp_Section_Update`, `WebApp_Section_Update-Code`

Create empty placeholders:
- `{AppRoot}/User/section/{NameNormalized}.php`
- `{AppRoot}/User/css/{NameNormalized}-Section.css`

Set CSV columns:
- `Code_Path = {AppRoot}/User/section/{NameNormalized}.php`
...
### Component group
Worklflow in:
- `WebApp_Component`, `WebApp_Component-Code`, `WebApp_Component_Update`, `WebApp_Component_Update-Code`

Create empty placeholders:
- `{AppRoot}/User/component/{NameNormalized}.php`
- `{AppRoot}/User/css/{NameNormalized}.css`

Set CSV columns:
- `Code_Path = {AppRoot}/User/component/{NameNormalized}.php`
- `Content_Path = {AppRoot}/User/css/{NameNormalized}.css`

### Layout group
Worklflow in:
- `WebApp_Layout`, `WebApp_Layout-Code`

Create empty placeholders:
- `{AppRoot}/User/layout/{NameNormalized}.php`
- `{AppRoot}/User/css/{NameNormalized}-Layout.css` (optional; create only when planner row intends layout CSS)

Set CSV columns:
- `Code_Path = {AppRoot}/User/layout/{NameNormalized}.php`
- `Content_Path = {AppRoot}/User/css/{NameNormalized}-Layout.css` (if layout CSS intended)

### WebSDK Module JS group (WebApp JS home; NON-NEGOTIABLE)
Worklflow in:
- `WebSDK_Module_JS`
- `WebSDK_Module_JS-Planning`
- `WebSDK_Module_JS-Code`
- `WebSDK_Module_JS-Documentation`

Notes:
- WebApp ke andar custom JS forbidden hai. JS ka home module-level hai.
- For this group, `Name` is the `{Module}` (no WebApp filename normalization).
- Validation: `{Module}` MUST be a simple module name (no `/`).

Do NOT create any WebApp local JS placeholders for this group.
...
- `-Planning` => set `Planning_Path` to:
  - `/Aliens/.Alien/{Developer}/Planning/WebApp/{AppName}/{Entity}/{NameNormalized}.md`
- `-Documentation` => set `Doc_Path` to the workflow’s documented output path (example for screens):
  - `/Aliens/Docs/WebApp/{AppName}/Screens/{NameNormalized}.md`
