---
WorkflowId: WebApp_Layout-Planning
Type: Workflow_Singular-Planning
Domain: WebApp
Scope: Generate planning for a WebApp Layout part (per target)
Status: Draft
Version: 26.00.00
...
ExampleFiles:
  ...
  - "/Aliens/WebApp/AlienCyborg/25.00.00/theme/AlienCyborg_Basic/User/layout.php"
Inputs:
  Workflow():
    - WorkflowId: "WebApp_Layout-Planning"
...
Outputs:
  - PlanningFile: "/Aliens/.Alien/{Developer_Username}/Planning/WebApp/{AppName}/Layout/{LayoutNameNormalized}.md"
NonNegotiables:
...
Planning ka output aage `WebApp_Layout-Code` aur `WebApp_Layout-Documentation` reuse karenge.
...
2) **Include/Usage Contract**
   - Layout parts are included by `User/layout.php`.
...
Plan exact outputs:
1) `{AppRoot}/User/layout/{LayoutNameNormalized}.php`
2) Optional: `{AppRoot}/User/css/{LayoutNameNormalized}-Layout.css`

No other files.

## Step E: Acceptance Checklist (Minimal)
- File naming follows `WebApp_Naming_and_Screens` SSOT
