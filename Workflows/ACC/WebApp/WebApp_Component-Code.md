---
WorkflowId: WebApp_Component-Code
Type: Workflow_Singular-Code
Domain: WebApp
Scope: Generate a WebApp Component + its CSS file (create-only). For plural components, also generate a singular child item component.
Status: Draft
Version: 26.00.00
...
Outputs:
  - ComponentFile: "/Aliens/WebApp/{AppName}/{LatestVersion}/theme/{AppName}_Basic/User/component/{ComponentNameNormalized}.php"
  - ChildComponentFile (PluralOnly): "/Aliens/WebApp/{AppName}/{LatestVersion}/theme/{AppName}_Basic/User/component/{ChildComponentNameNormalized}.php"
  - CssFile: "/Aliens/WebApp/{AppName}/{LatestVersion}/theme/{AppName}_Basic/User/css/{ComponentNameNormalized}.css"
NonNegotiables:
...
- ComponentOut: `{AppRoot}/User/component/{ComponentNameNormalized}.php`
- CssOut: `{AppRoot}/User/css/{ComponentNameNormalized}.css`

No double prefix:
- Never output `AppName.AppName.X.php` / `.css`
...
- If ComponentOut exists OR CssOut exists:
  ...
  - Action: run `WebApp_Component_Update` workflow
...
- Otherwise fail with:
  ...
  - Action: run `WebApp_Component-Planning`

---

# [5] Code Generation Rules (Enterprise + Premium)
## 5.1 AliensStyle (Latest)
Component PHP file must include:
- `<?php Access(); ?>` at top
...
- Component files MUST NOT declare any PHP functions (`function X(){}`), anonymous functions, closures, or classes.
- Utilities priority order:
  1) Use `WebSDK_Module_*` for business + reusable helpers.
  2) If a tiny utility is genuinely needed, it must live in WebOS System function folder (not inside WebApp components).
...
- Yeh markers WebOS hydration engine ke liye hain.
...
- WebOS `Component()` helper global `$Array` + `$Screen` reset karta hai (Kernel behavior).
...
Component PHP me **maximum processing** (conditions/loops/array_merge/htmlspecialchars/component calls) sirf is guard ke andar ho:
...
- Default dataset: jab component no payload ke sath Screen render me call ho (WebOS `Component()` global `$Screen=true` set karta hai).
