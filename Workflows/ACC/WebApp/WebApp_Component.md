---
WorkflowId: WebApp_Component
Type: Workflow_Plural
Domain: WebApp
Scope: Generate WebApp Components (planning + code + docs) via child workflows
Status: Draft
Version: 26.00.00
...
# [0] Purpose
Yeh workflow **1 ya multiple WebApp Components** generate karwata hai.
...
Example:
- Workflow( "WebApp_Component", "Exchange/Deposit_Transactions, Social/Post", "UI premium hona zarori hai" )
...
- Default Version: latest numeric WebApp version folder (example: `26.00.00`)
...
- Component files are stored under App Theme:
  - Component: `User/component/{ComponentNameNormalized}.php`
  - CSS: `User/css/{ComponentNameNormalized}.css`

## 2.3.1 JS Template-System Hydration Contract (Mandatory)
Component HTML must remain compatible with WebOS JS hydration (Unit/js/001.js):
- Note: yeh **HTML marker contract** hai (attributes + placeholders) — WebApp ke andar custom JS likhna allowed nahi.
...
- If user passes `AlienCyborg/WithdrawNew`, then **generated filenames** must be:
  - `.../User/component/AlienCyborg.WithdrawNew.php`
...
- Guard fail ho (Screen false ya `$Array` empty) to:
  ...
  - goal: backend integration na ho to PHP load minimum rahe
...
Call:
- Workflow( "WebApp_Component-Planning", "{AppName}/{ComponentName}", "{Description}" )
...
If user wants extra:
- Run a different workflow (e.g., `WebApp_Template`, `WebApp_Structure_Update`, etc.)
