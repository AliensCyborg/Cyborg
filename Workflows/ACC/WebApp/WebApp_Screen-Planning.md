---
WorkflowId: WebApp_Screen-Planning
Type: Workflow_Singular-Planning
Domain: WebApp
Scope: Generate planning for a WebApp Screen (per target)
Status: Draft
Version: 26.00.00
...
Outputs:
  - PlanningFile: "/Aliens/.Alien/{Developer_Username}/Planning/WebApp/{AppName}/Screen/{ScreenNameNormalized}.md"
NonNegotiables:
...
Planning ka output aage `WebApp_Screen-Code` aur `WebApp_Screen-Documentation` reuse karenge.
...
2) **ScreenScopeTag (UTag)** (required)
  ...
  - Non-negotiable: it must be globally unique across the entire WebOS ecosystem.
  - Allocate via central registry (no manual pick): `Workflows/ACC/_Refs/WebApp_UTag.md`
...
- `MissingSections[]` (must be created via WebApp_Section before screen code)
