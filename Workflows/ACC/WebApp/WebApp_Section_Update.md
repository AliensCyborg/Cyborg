---
WorkflowId: WebApp_Section_Update
Type: Workflow_Plural
Domain: WebApp
Scope: Update existing WebApp Sections (planning + code + docs) via child workflows (modify-only)
Status: Draft
Version: 26.00.00
...
# [0] Purpose
Yeh workflow **existing WebApp Sections** ko update karwata hai.
...
Example:
- Workflow( "WebApp_Section_Update", "AlienCyborg/Deposit_Header, AlienCyborg/Home_Hero", "Layout refine" )
...
Call:
- Workflow( "WebApp_Section_Update-Audit", "{AppName}/{SectionName}", "{Description}" )

## Step B: Planning (Child Workflow)
Call:
- Workflow( "WebApp_Section_Update-Planning", "{AppName}/{SectionName}", "{Description}" )
...
- For each required component:
  ...
  - If missing: pehle `WebApp_Component` workflow run karo (loop / per component), phir section update apply karo.
...
- Action: run `WebApp_Component` for missing components, then re-run section update.

## Step C: Code Update (Child Workflow)
Call:
- Workflow( "WebApp_Section_Update-Code", "{AppName}/{SectionName}", "{Description}" )

## Step C.5: Reaudit (Post-Code Verification, Read-only)
- Workflow( "WebApp_Section_Update-Reaudit", "{AppName}/{SectionName}", "{Description}" )
...
Call:
- Workflow( "WebApp_Section_Update-Documentation", "{AppName}/{SectionName}", "{Description}" )
