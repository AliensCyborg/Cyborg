---
WorkflowId: WebApp-Development
WorkflowName: WebApp Development
Type: Workflow_Plural
Domain: WebApp
Scope: Execute Planner CSV tasks in priority order; update task status codes
Status: Draft
Version: 26.00.00
...
# [0] Purpose
`Workflow("WebApp-Development", "{AppName}", "{Description}")`

Read the Planner CSV and execute tasks in priority order, updating status codes.
...
- For each task:
  ...
  - If `Task.Worklflow` is a Screen/Section/Component group (create/update), then `Task.utag` MUST be non-empty.
...
- Write run summary under:
  - `/Aliens/.Alien/{Developer}/Runs/WebApp/{AppName}/WebApp-Development.Run.md`
