---
WorkflowId: WebApp_Section_Update-Code
Type: Workflow_Singular-Code
Domain: WebApp
Scope: Update a WebApp Section + its CSS file (modify-only)
Status: Draft
Version: 26.00.00
...
- SectionNameNormalized (canonical; SSOT): prefix rule per `Workflows/ACC/_Refs/WebApp_Naming_and_Screens.md` (and must contain underscore)
...
- Action: run `WebApp_Section_Update-Planning`
...
- Inline PHP islands are allowed in sections (matches existing theme style).
...
- Agar planning me required component missing ho:
  ...
  - `WebApp_Component` workflow run karke missing components create karo (same theme), phir section update apply karo.
...
- Components create karna sirf dependency step ke tor par `WebApp_Component` workflow se allowed hai.
