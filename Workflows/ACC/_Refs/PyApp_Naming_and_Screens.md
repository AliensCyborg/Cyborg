...
Purpose: >
  PyApps theme me Component/Section/Screen/Template naming + prefix rules.
...
# PyApp — Naming, Prefixing, and Screens (SSOT)

## [0] Core UI Hierarchy (SSOT)
PyApps ke andar UI artifact hierarchy fix hai:

1) **Components**
2) **Sections** (compose components)
3) **Screens** (compose sections)
4) **PyApp** (screens host)
...
- **PyApp** actual me multiple screens ko host karta hai (routing/nav/wiring), screen ke andar ka UI structure nahi todta.
...
- Composer rule:
  ...
  - PyApp/page: sirf screens host kare
...
- Output artifacts MUST use the normalized name in their filenames (Python + CSS + docs paths).

Examples:
- App=`B-Commerce`, Name=`B-Commerce.Home` => Screen file: `B-Commerce.Home.py`
...
### 2.2 Screen File Naming (Python)
Rule:
- `{{AppName}}.{{ScreenName}}.py`
...
## [3] Component Naming
### 3.1 Component Python
- `{{AppName}}.{{ComponentName}}.py` (normalized)
...
- App: `B-Commerce`
  - `B-Commerce.User_ProfilePicture.py`
  - `B-Commerce.User_CoverImage.py`
  - `B-Commerce.User_Addresses.py`
  - `B-Commerce.User_Address.py`
  - `B-Commerce.User_ProfilePicture.css`
...
UTag note:
- UTag allocation + format SSOT: `Workflows/ACC/_Refs/PyApp_UTag.md`
...
## [4] Section Naming
- `{{AppName}}.{{ScreenName}}_{{SectionName}}.py` (normalized)
...
- App: `B-Commerce`
  - `B-Commerce.Home_Header.py`
  - `B-Commerce.Home_Hero.py`
...
Rule (layout part Python):
- `{{AppName}}.{{LayoutPartName}}.py`
...
- Entry/aggregator files like `User/layout.py`, `User/css.py`, `User/js.py`, `User/component.py` are framework files and are not renamed by this rule.

Layout examples (strict):
- App: `B-Commerce`
  - `B-Commerce.Nav.py`
  - `B-Commerce.Header.py`
  - `B-Commerce.Footer.py`
...
- Default dataset: jab component **no payload** ke sath Screen render me call ho (PyOS `Component()` global `$Screen=true` set karta hai), tab component default dataset render kar sakta hai.
...
- UTag collisions are forbidden across the entire PyOS ecosystem (all PyApps).
