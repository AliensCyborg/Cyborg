...
Summary: >
  Plural workflow that produces a complete WebSDK Module by orchestrating multiple child workflows.
...
CallsNow:
  - "Workflows/ACC/WebSDK/WebSDK_Module_Class.md"
  - "Workflows/ACC/WebSDK/WebSDK_Module_API.md"
CallsLater:
  - "Workflows/ACC/WebSDK/WebSDK_Module_SQL.md"
  - "Workflows/ACC/WebSDK/WebSDK_Module_Cron.md"
  - "Workflows/ACC/WebSDK/WebSDK_Module_Webhook.md"
  - "Workflows/ACC/WebSDK/WebSDK_Module_Component.md"
  - "Workflows/ACC/WebSDK/WebSDK_Module_Screen.md"
  - "Workflows/ACC/WebSDK/WebSDK_Module_JS.md"
Inputs:
  - Name: Targets
    Type: string
    Required: true
...
## Purpose
`WebSDK_Module` ek **Plural** workflow hai jo ek WebSDK **Module** (example: `Teacher`) ko complete banata hai — by calling multiple child workflows.

Example:
- `Workflow("WebSDK_Module", "Teacher", "Teacher module complete banao")`

Agar multiple modules:
- `Workflow("WebSDK_Module", "Teacher,User,Account", "In modules ko setup karo")`
...
1) `Workflow("WebSDK_Module_Class", "<ModuleName>", "<Description>")`
2) `Workflow("WebSDK_Module_API", "<ModuleName>", "<Description>")`

In dono ke complete hote hi module usable state me aa jana chahiye (class + APIs ready).

### Important (Module-only contract)
- `WebSDK_Module` MUST call `WebSDK_Module_API` with **module-only target** (NO slash).
  - This intentionally activates `WebSDK_Module_API` module-only mode (batch create) which expands to minimum CRUD endpoints.
  - The CRUD minimum is governed by `WebSDK_Module_API-*` workflows (not duplicated here).
- `WebSDK_Module` MUST call `WebSDK_Module_Class` with **module-only target** (NO slash).
  - Class workflow is responsible for creating/updating `/php/index.php`.
...
3) `WebSDK_Module_SQL`
4) `WebSDK_Module_Cron`
5) `WebSDK_Module_Webhook`
6) `WebSDK_Module_Component`
7) `WebSDK_Module_Screen`
8) `WebSDK_Module_JS`
...
### Step 2 — Resolve WebSDK version
- Use latest available WebSDK version automatically
- If auto decide possible nahi:
  - fallback: `PHP.26.00.00`

### Step 3 — Resolve module root path
For `{ModuleName}`:
- `{ModuleRoot} = /Aliens/WebSDK/{PHP.Version}/Module/{ModuleName}/`
