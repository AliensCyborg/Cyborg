---
WorkflowId: WebSDK_Module_API-Planning
WorkflowName: WebSDK Module API Planning
...
**Role (Hinglish):**  
Yeh file `WebSDK_Module_API` workflow ke liye **Planning generator** hai.  
...
> Important: Is file me sirf **unique planning rules** honge jo *WebSDK Module APIs* ke liye specific hain.  
...
- Left side (before `/`) = **WebSDK Module name**
...
### WebSDK root (default latest)
- `/Aliens/WebSDK/PHP.{Latest}/Module/{Module}/`

If auto latest decide na ho paye:
- default: `PHP.26.00.00`

### Target module folder
- `{ModuleRoot} = /Aliens/WebSDK/PHP.26.00.00/Module/{Module}/`
...
- Planning defaults to:
  ...
  - primary entry class in `php/index.php` (modular layout optional; enabled if large or requested)
...
- Planning MUST:
  ...
  - enforce base table requirement: `{Module}` table is mandatory in every module
    - if `{Module}_*` part tables are involved but base `{Module}` table is missing => add blocker `BLOCKER_BASE_TABLE_MISSING` and require `WebSDK_Module_SQL_Update`
  - explicitly map which module-class method(s) handle which unit
  - assume modular layout exists/should exist: `index.php` + `php/_Trait.php` + `php/{Module}_{Part}.php`
...
- If any **new table or column** is required by the API or class method changes:
  - MUST require `WebSDK_Module_SQL_Update` for the same module.
...
Planner must list:
- Which WebOS engines are likely needed (Auth, DB, Cache, Validate, RateLimit, Log, etc.)
...
- If planner decides any new/updated class method is required:
  - MUST declare dependency on `WebSDK_Module_Class_Update`.
...
- no direct SQL injection risk (use WebOS DB engine patterns)
...
- API `{ApiName}.php` should call module class method `{ApiName}()` (same identifier) to keep deterministic cross-layer naming.
- If method missing in module class, planner MUST add it as a dependency (create via `WebSDK_Module_Class` / add method via class code workflow), not implement logic inside API.
...
- `Workflow("Memory-Read", "WebSDK.Module.{Module}", "limit=10")`

---

## [07] Examples (interpretation only)
### Example 1
`Workflow("WebSDK_Module_API", "User/UserNew", "New user create api with validation and proper errors")`
Planning must:
- resolve `/Aliens/WebSDK/PHP.26.00.00/Module/User/api/UserNew.php`
- determine create/update
- map table from `User/sql/index.sql`
- define input fields and auth strategy

### Example 2
`Workflow("WebSDK_Module_API", "Web3/Web3_Accounts", "List accounts with pagination, secure by default")`
Planning must:
...
- `WebSDK_Module_API-Code.md` can generate correct API file(s)
- `WebSDK_Module_API-Documentation.md` can generate docs in `/Aliens/Docs/WebSDK/...`
