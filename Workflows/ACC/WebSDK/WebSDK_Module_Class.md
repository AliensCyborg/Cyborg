---
WorkflowId: WebSDK_Module_Class
WorkflowName: WebSDK Module Class
Type: Workflow_Plural
Domain: WebSDK
Category: Module
Intent: "Ensure/Generate a module primary class (index.php) for one or multiple modules"
Status: Draft
Version: 26.00.00
...
TargetRules:
  ...
  Meaning:
    ...
    - "One slash => create a Module Part file (php/{FileBase}.php) and wire it into index.php"
  ModuleTypeRule:
...
Outputs:
  Planning:
    Location: "/Aliens/.Alien/{Developer_Username}/Planning/WebSDK/Module/{ModuleName}/Class.md"
  Code:
...
**Purpose (Hinglish):**  
Yeh workflow WebSDK ke kisi bhi Module ka **primary class** (usually `php/index.php`) ko **generate/update** karne ke liye hai.  
...
3) Code workflow se `php/index.php` generate/update karwayega.
4) Documentation workflow se docs generate karwayega (module class overview + usage).

---

## [02] What this workflow DOES NOT do
- Module folder create karna (wo `WebSDK_Module` ka kaam hai).
...
- It will create `/php/W3_Token_Wallet.php` (class `W3_Token_Wallet`) and wire it into `php/index.php` so the main module object can access it.
- If you want to update only an existing part file, prefer `WebSDK_Module_Class_Update` with `Module/FileBase`.
...
1) Resolve LatestVersion:
   - Prefer “latest” policy (fallback PHP.26.00.00 if auto decide not possible).
...
3) Must exist:
   - If missing => error: “Module not found. Run WebSDK_Module first or create module structure.”
4) Identify existing assets (optional but recommended):
   - `php/index.php` (existing class)
...
Actual unique code rules live in `WebSDK_Module_Class-Code.md`, but parent workflow MUST enforce:
...
- **Database prefix law**: never hardcode full table names (e.g., `Coinpods_...`) in WebSDK modules; use base names only (see `Workflows/ACC/_Refs/WebSDK.md`).
...
- Primary entry point remains: `/php/index.php`
- Facade traits file: `/php/_Trait.php`
- Partition/part files: `/php/{ModuleName}_{Part}.php`
...
SSOT details:
- Planning: `WebSDK_Module_Class-Planning.md` (`PartitionPlan.Enabled`)
- Code: `WebSDK_Module_Class-Code.md` (require/use/delegation rules)
- Update: `WebSDK_Module_Class_Update-*` (audit recommendation + migration plan + non-destructive move rules)

If existing `php/index.php` found:
...
If missing:
- Create new `php/index.php` with full required structure.

Create vs Update clarity (Strict):
- This workflow (`WebSDK_Module_Class`) is allowed to create a missing `php/index.php`.
- Update-only workflow (`WebSDK_Module_Class_Update`) MUST error if `php/index.php` is missing.
...
- File map of module folder (php/api/cron/webhook/sql)
...
- `$PHP['{Module}']['function'][]` list must cover every public entry point

---

## [10] Quick usage examples
1) Two modules:
`Workflow("WebSDK_Module_Class", "User, Account", "Class ko enterprise grade banana hai")`

2) Plural module (multi-table + units):
`Workflow("WebSDK_Module_Class", "User", "Plural module hai (User + User_Address + User_Meta). Modular layout enforce karo")`
