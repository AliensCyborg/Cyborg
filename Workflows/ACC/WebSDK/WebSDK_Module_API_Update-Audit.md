---
WorkflowId: WebSDK_Module_API_Update-Audit
WorkflowName: WebSDK Module API Update (Audit)
...
Outputs:
  Audit:
    ...
    FileName: "Audit.md"
  Artifacts:
    SaveTo: "/Aliens/.Alien/{Developer_Username}/Planning/WebSDK/Module/{Module}/API/"
...
# WebSDK_Module_API_Update-Audit
...
## [00] Purpose
Is workflow ka kaam: **enterprise-grade audit** of an existing WebSDK Module API before update.
...
- Also write a stable copy for downstream steps:
  - `/Aliens/.Alien/{Developer}/Planning/WebSDK/Module/{Module}/API/{ApiName}.update.audit.md`
...
Batch mode:
- If module-only, enumerate `{ModuleRoot}/api/*.php` (non-recursive). If none => Completed(NoTargets).
...
### B) WebSDK API baseline compliance
