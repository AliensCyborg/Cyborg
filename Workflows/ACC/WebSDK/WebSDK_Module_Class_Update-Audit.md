---
WorkflowId: WebSDK_Module_Class_Update-Audit
WorkflowName: WebSDK Module Class Update (Audit)
...
## [00] Purpose
Is workflow ka kaam: **enterprise-grade audit** of an existing WebSDK module class file (`php/index.php`) before update.
...
- Also write a stable copy for downstream steps:
  - `/Aliens/.Alien/{Developer}/Planning/WebSDK/Module/{ModuleName}/Class.Update.audit.md`
...
- `ModuleName` => audit ALL php class files under module:
  - `/Aliens/WebSDK/{LatestVersion}/Module/{ModuleName}/php/*.php`
  - (minimum required coverage: `index.php` + `_Trait.php` (if exists) + `{ModuleName}_*.php` files)
...
- If target is `ModuleName`:
  - `{PhpRoot} = {ModuleRoot}/php/`
  - `{ClassFiles} = {PhpRoot}/*.php` (enumerate files; deterministic ordering: `index.php` first, then `_Trait.php`, then remaining sorted)
- If target is `ModuleName/FileBase`:
  - `{PhpRoot} = {ModuleRoot}/php/`
  - `{ClassFile} = {PhpRoot}/{FileBase}.php`
- Optional: `{ModuleSql} = {ModuleRoot}/sql/index.sql`

Hard rule:
- If target is `ModuleName` and `{PhpRoot}` missing OR no `.php` files => fail with `WF_TARGET_MISSING`.
...
- If target is `ModuleName/FileBase` => write:
  - `/Aliens/.Alien/{Developer}/Planning/WebSDK/Module/{ModuleName}/Class.Update.{FileBase}.audit.md`
...
- `$PHP['{Module}']` metadata + `$PHP['{Module}']['function'][]` manifest present/consistent (best-effort)
...
- If the class/method/table family indicates plural units (any of these is enough):
  ...
  - Module php folder contains any `{ModuleName}_*.php` files
  => Set `ModuleType=PluralModule`.
- Else => `ModuleType=SingularModule`.

2) Large file heuristic (SingularModule only):
- If `php/index.php` is `> 1500 lines` OR has `> 80 public methods` OR file size `> 200 KB` => set `PartitionRecommendation=RECOMMENDED`.
