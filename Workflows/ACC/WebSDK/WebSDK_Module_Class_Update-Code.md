---
WorkflowId: WebSDK_Module_Class_Update-Code
WorkflowName: WebSDK Module Class Update Code
...
Outputs:
  UpdatedFile:
    Location: "/Aliens/WebSDK/{LatestVersion}/Module/{ModuleName}/php/index.php"
...
- If target is `ModuleName`, updated files are:
  - `/Aliens/WebSDK/{LatestVersion}/Module/{ModuleName}/php/*.php` (modify-only; no create)
Safety:
  NoDelete: true
  ModifyOnly: true
...
# WebSDK_Module_Class_Update-Code.md
...
## [0] Purpose
Yeh workflow existing module php classes ko **update** karta hai (modify-only).

Target meaning (NEW SSOT):
- `ModuleName` => update ALL PHP class files in module `php/` folder:
  - `/Aliens/WebSDK/{LatestVersion}/Module/{ModuleName}/php/*.php`
...
When invoked as part of `WebSDK_Module_Class_Update`, it MUST execute a planning-derived checklist and must not stop after applying only a small subset of rules.

## [0.1] Hard Gates Checklist (Non-negotiable)
These checks must be satisfied in every run (even when Description is blank):
- Update-only: never create missing target php file(s) (error instead).
- Header sanity: exactly one `<?php` at file start; no duplicate open tags; no `?>` close tag.
- Canonical ordering (index.php only): if the file being updated is `php/index.php`, ensure `declare(strict_types=1);` then `Access();` at top-of-file when safely possible.
- Non-index files (`_Trait.php`, `{ModuleName}_*.php`, other php/*.php): do NOT force `Access();` insertion unless the file already follows that pattern; keep changes minimal and non-breaking.
...
- Syntax gate: `php -l` must pass for each updated file.
...
- Keep output short: `Done: <BatchName>. php -l <OK/FAIL>.`
...
- NEVER remove existing `$PHP['{Module}']['function'][]` manifest entries.
- Changes must be applied in multiple small, reviewable batches.

Mandatory artifacts (per run item):
- `Before.index.php` snapshot (full file copy)
- `After.index.php` snapshot (full file copy)
...
- WebSDK/WebOS refs (correct engine/system usage)
...
- Prior to each method divider, insert a comment micro-header capturing Developer, LastTouched date, Inputs, Outputs, and Dependencies. Example:
  ...
  - `// Output    : array|null`
  - `// Dependencies : Row(), UID_Encode(), WebOS.Auth, WebOS.Validate`
...
- Fallback: `PHP.26.00.00` (agar auto detect possible nahi)
- Resolve:
  - ModuleRoot: `/Aliens/WebSDK/{LatestVersion}/Module/{ModuleName}/`
  - ClassFile: `/php/index.php` (mandatory)

## [4] Preconditions (No guessing)
Hard preconditions:
- Module folder exists
- Class file exists
- `/php/` folder exists (expected structure)
- Read permission ok
...
- WebSDK ke 4 reference modules/classes dhundo (same ecosystem, similar patterns)
- Prefer:
...
- Pick 4 best matches:
  ...
  - 2 plural modules (modules with `{ModuleName}_*.php` parts and/or multiple `{ModuleName}_*` tables) if available
...
**Files involved (all under module php/):**
- `index.php` (existing; modify-only)
- `_Trait.php` (create-or-update)
- `{ModuleName}_{Part}.php` (create-or-update per part)
...
- `php -l` must pass after each batch.

**Execution order (must be used):**
1) Ensure part class files exist (create if missing).
2) Ensure `_Trait.php` exists (create if missing) and contains facade traits.
3) Patch `index.php`:
  - Insert `require_once __DIR__ . '/{ModuleName}_{Part}.php';` (for each part)
  - Insert `require_once __DIR__ . '/_Trait.php';`
...
- If `index.php` is missing => `ERR_CLASS_FILE_NOT_FOUND` (do not create).
- If `php/` folder missing => `ERR_MODULE_NOT_FOUND` (or a more specific structure error) (do not create).
...
This update workflow MUST preserve (or safely restore) the canonical WebSDK module class ordering as seen in reference modules like:
...
0) Exactly one `<?php` open tag at file start (no duplicate open tags; no `?>` close tag).
...
7) `$PHP['{Module}']` metadata + `$PHP['{Module}']['function'][]` manifest
8) `class {ModuleName} { ... }`
...
- Renaming existing `$PHP['{Module}']` keys (preserve existing `Created/modified/by` keys; only add missing keys).

Additional enforcement (safe + non-breaking):
- If the module already has `$PHP['{Module}']['function'][]` manifest entries:
...
$PHP['{Module}']['modified'] = 'YYYY/MM/DD';
$PHP['{Module}']['by'] = 'AlienCyborg';
$PHP['{Module}']['function'][] = '__construct()';
...
- If (and only if) Description explicitly requires adding missing CRUD surface:
  ...
  - New methods must be added in a backward-compatible way and registered in `$PHP['{Module}']['function'][]`.
...
This workflow MUST follow the checklist produced by `WebSDK_Module_Class_Update-Planning`.
...
- Do not introduce new `static` methods or static dispatch patterns in WebSDK module classes.
...
If `/Aliens/WebSDK/{LatestVersion}/Module/{ModuleName}/sql/index.sql` exists:
...
- Do not write `/Aliens/WebSDK/...` until approved
Block conditions:
...
Success tabhi:
- index.php valid PHP syntax (no parse errors)
