---
WorkflowId: WebSDK_Module_Class-Planning
WorkflowName: WebSDK Module Class Planning
...
Outputs:
  Planning:
    SaveTo: "/Aliens/.Alien/{Developer_Username}/Planning/WebSDK/Module/Class/"
    Format: "md"
Safety:
  NoDelete: true
...
# WebSDK_Module_Class-Planning.md

## [00] Purpose
Yeh workflow **sirf planning generate** karta hai for:  
`Workflow("WebSDK_Module_Class", "User, Account", "...")`

Meaning:
- Targets me jo bhi items hain (User, Account, Web3, etc.) unke liye **module ki main class** (`php/index.php`) ko analyze + plan banana.
...
> Agar module folder hi missing ho, to **error**. Module create karne ke liye `WebSDK_Module` workflow use hoga.
...
- Version: **latest** (fallback `PHP.26.00.00` if auto decide possible nahi)
- WebSDK Root:
  - `/Aliens/WebSDK/{Version}/Module/`
...
- Class file (primary):
  - `/Aliens/WebSDK/{Version}/Module/{ModuleName}/php/index.php`
...
Note:
- “Modify-only, missing ho to error” rule applies to `WebSDK_Module_Class_Update` workflow, not to this create workflow.
...
2) If SQL missing: inspect module php folder
   - If any file matches `{ModuleName}_*.php` => `ModuleType = PluralModule`
3) If still unclear: inspect existing method names in `php/index.php`
...
- Plan MUST still enforce: “Har module ke liye DB table required hai”; therefore add a blocker note: “Define/Update tables via WebSDK_Module_SQL_Update before finalizing code behavior.”
...
2) If SQL missing:
  - Heuristic:
    ...
    - If any `{ModuleName}_*` part units are detected from php/methods => include those part tables in `Tables[]` as *required-by-contract* and mark as `TABLE_REFERENCE_INFERRED` (must be confirmed via SQL update).
...
- If SQL/inspection detects `{ModuleName}_*` tables but base `{ModuleName}` is missing:
  ...
  - Include fix instruction: “Add base table `{ModuleName}` via `WebSDK_Module_SQL_Update` before proceeding with code changes.”

Database prefix law (planning-level):
- Planning MUST store **base table names** for WebSDK module code generation (no hardcoded prefix).
...
Planning must scan current `php/index.php` to decide:
...
- Main class (entrypoint; backward compatible):
  - `{MainClassFile} = /Aliens/WebSDK/{Version}/Module/{ModuleName}/php/index.php`
- Facade traits file (wrapper methods):
  - `{TraitFile} = /Aliens/WebSDK/{Version}/Module/{ModuleName}/php/_Trait.php`
- Partition classes (feature chunks):
  - `{PartFilePattern} = /Aliens/WebSDK/{Version}/Module/{ModuleName}/php/{ModuleName}_{Part}.php`

Examples for `User`:
- `php/index.php`
- `php/_Trait.php`
- `php/User_Address.php` (class `User_Address`)
- `php/User_Meta.php` (class `User_Meta`)
...
- Else if `ModuleType = SingularModule`:
  - Enable if ANY of these is true:
    - Existing `php/index.php` is large (recommended heuristic): `> 1500 lines` OR `> 80 public methods` OR `> 200 KB`
...
### E) Extensibility
- Hooks points: Before/After actions (if your WebOS pattern supports)
- Meta strategy (if module uses meta tables)

### F) Cross-Layer Naming Alignment (SSOT; deterministic)
When a module is used end-to-end (WebSDK Class + WebSDK API + WebApp UI):
...
- WebApp UI artifacts compose/render only; business logic still lives in WebSDK.
...
- Module scope: `/Aliens/.Alien/{Developer}/Planning/WebSDK/Module/Class/{ModuleName}.md`
- File scope: `/Aliens/.Alien/{Developer}/Planning/WebSDK/Module/Class/{ModuleName}.{FileBase}.md`
...
- Clearly state whether this target is `CreateScenario` (index.php missing) or `UpdateScenario` (index.php exists).
- Include an explicit checklist for AliensStyle rules to enforce in code generation:
  ...
  - `$PHP['{Module}']['function'][]` manifest completeness
  - No `function_exists()` guards for core primitives in WebSDK modules
