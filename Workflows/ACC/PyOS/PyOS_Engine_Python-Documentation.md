---
WorkflowId: PyOS_Engine_Python-Documentation
WorkflowName: PyOS Engine Python Documentation
...
Output:
  ...
  DocOutputRelPath: "Engine/py/{EngineBaseName}.md"
...
## [00] Purpose
Is workflow ka kaam: PyOS Python Engine ki **documentation generate/update** karna.
...
Examples:
- `Workflow("PyOS_Engine_Python", "PyOS.Cache", "Docs + examples" )`
- `Workflow("PyOS_Engine_Python", "PyOS.Troubleshoot", "Docs for new engine" )`
...
Additionally (PyOS Engine specific):
- Router Mapping
  - `PyOS::{EngineShortName}()` → `{ClassName}::Main($Payload,$Context)`
  - `PyOS::{EngineShortName}{Action}()` → `{ClassName}::{Action}($Payload,$Context)`
...
- Include payload calling patterns:
  - `PyOS::{EngineShortName}($arg1, $arg2, ...)` → `$Payload = [$arg1,$arg2,...]` (EngineRouter style)
...
- Dependencies section MUST list:
  - PyOS engines actually used (Security/Policy/Log/Cache/etc)
