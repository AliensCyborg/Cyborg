---
WorkflowId: PyOS_Engine_SQL
WorkflowName: PyOS Engine SQL (Orchestrator)
...
Outputs:
  - Planning: "/Aliens/.Alien/{Developer_Username}/Planning/PyOS/Engine/SQL/{EngineBaseName}.plan.md"
  - Code: "/Aliens/PyOS/{Version}/unit/sql/{EngineBaseName}.sql"
  - Documentation: "/Aliens/Docs/PyOS/Engine/SQL/{EngineBaseName}.md"
Requires:
...
ChildWorkflows:
  - PyOS_Engine_SQL-Planning
  - PyOS_Engine_SQL-Code
  - PyOS_Engine_SQL-Documentation
...
...
## [00] Purpose
Is workflow ka kaam: ek ya multiple PyOS SQL engines ke liye **Planning + Code + Documentation** generate/update karwana,
strictly PyOS ecosystem rules ke hisaab se.
...
## [01] Invocation
Signature:
- `Workflow("PyOS_Engine_SQL", "PyOS.Cache, PyOS.Auth", "Aese kr do wese kr do")`
...
Fallback:
- If auto-detect is not possible, default to `Py.26.00.00` (as per project rule).

If `/Aliens/PyOS/{Version}` is missing:
- Error (do not create PyOS installation folders in this workflow).

## [04] Execution Flow (Deterministic)
For each Target (in given order):

1) Call Planning:
- `Workflow("PyOS_Engine_SQL-Planning", "<Target>", "<Description>")`

2) Call Code:
- `Workflow("PyOS_Engine_SQL-Code", "<Target>", "<Description>")`

3) Call Documentation:
- `Workflow("PyOS_Engine_SQL-Documentation", "<Target>", "<Description>")`
...
- Allowed write scopes (PyOS/PyApps/PyBrand)
...
CSV mode mirrors the standard PyOS CSV rules for create/update (Planning → Code → Documentation).

### [10.1] Invocation
Allowed:
- `Workflow('PyOS_Engine_SQL','PyOS.Cache')`
- `Workflow('PyOS_Engine_SQL','{FILE_NAME}.csv')`
