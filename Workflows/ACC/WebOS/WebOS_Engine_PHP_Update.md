---
WorkflowId: WebOS_Engine_PHP_Update
...
EntryPoint: true
Purpose: >
  Plural update orchestrator: given one-or-more existing WebOS PHP engine names (without extension),
...
Invocation:
  - Workflow("WebOS_Engine_PHP_Update", "WebOS.Cache", "Performance + security hardening; backward compatibility maintained")
  - Workflow("WebOS_Engine_PHP_Update", "WebOS.Cache, WebOS.Auth", "Upgrade engines safely")
  - Wrokflow("WebOS_Engine_PHP_Update", "{FILE_NAME}.csv")
TargetsSyntax:
...
Outputs:
  ...
  Code:
    Path: "/Aliens/WebOS/PHP.{Latest}/Engine/"
...
Alignment rule (hard):
- This workflow must stay aligned with the latest rules in `Workflows/ACC/WebOS/WebOS_Engine_PHP.md`.
...
# [03] Target Parsing Rules (Same as WebOS_Engine_PHP)
...
- `.php` forbidden in normal mode targets
- Empty items => error

# [04] Update-Only Existence Gate (Hard)
For EACH EngineName:
1) Resolve engine version path using ACC.Paths rules (latest; fallback `PHP.26.00.00`).
2) Resolve file path:
...
### Step 1: Audit (Hard)
Call:
- Workflow("WebOS_Engine_PHP_Update-Audit", "{EngineName}", "{Description}")

Expected Output:
- `/Aliens/.Alien/{Developer_Username}/Runs/{RunId}/Items/{RunItemId}/Audit.md`

Deterministic Audit Artifact (hard):
- `/Aliens/.Alien/{Developer_Username}/Planning/WebOS/Engine/PHP/{EngineName}.update.audit.md`
...
Call:
- Workflow("WebOS_Engine_PHP_Update-Planning", "{EngineName}", "{Description}")

Expected Output:
- `/Aliens/.Alien/{Developer_Username}/Planning/WebOS/Engine/PHP/{EngineName}.update.plan.md`

### Step 3: Code (Update)
Call:
- Workflow("WebOS_Engine_PHP_Update-Code", "{EngineName}", "{Description}")
...
- All enterprise quality gates from `WebOS_Engine_PHP` apply (no skeleton outputs).

### Step 4: Reaudit (Post-Code Verification, Read-only)
Call:
- Workflow("WebOS_Engine_PHP_Update-Reaudit", "{EngineName}", "{Description}")

Expected Output:
- `/Aliens/.Alien/{Developer_Username}/Planning/WebOS/Engine/PHP/{EngineName}.update.reaudit.md`
...
Call:
- Workflow("WebOS_Engine_PHP_Update-Documentation", "{EngineName}", "{Description}")
...
CSV mode mirrors `WebOS_Engine_PHP` CSV rules, with one extra gate: target MUST exist.

## [07.1] Invocation
Allowed:
- `Workflow('WebOS_Engine_PHP_Update','WebOS.Cache')`
- `Wrokflow('WebOS_Engine_PHP_Update','{FILE_NAME}.csv')`  *(legacy typo supported)*
...
- Trim
- Strip extension `.php` / `.js` if present
...
### Stage A: Audit
- If `audit != 1` then run `WebOS_Engine_PHP_Update-Audit`.
- On success => `audit=1`.
- On fail => `audit=2`, `status=2`, skip remaining.

### Stage B: Planning
- If `planning != 1` then run `WebOS_Engine_PHP_Update-Planning`.
...
If exists and `code != 1` then run `WebOS_Engine_PHP_Update-Code`.
- On success => `code=1`.
- On fail => `code=2`, `status=2`, skip remaining.

### Stage D: Code Save / Verify
- Same verification rules as `WebOS_Engine_PHP` (file exists, lint passes, header/footer/baseline present).

### Stage E/F: Docs + Docs Verify
- Same rules as `WebOS_Engine_PHP`.
