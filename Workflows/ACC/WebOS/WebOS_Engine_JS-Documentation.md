---
WorkflowId: WebOS_Engine_JS-Documentation
...
Purpose: >
  Generate/update documentation for a single WebOS JS engine based on the final code (source of truth)
  and the plan (if present). Documentation must be consistent, accurate, and scoped to the request.
  NOTE: For existing-engine enterprise updates, use WebOS_Engine_JS_Update-Documentation so docs reflect
  the audit findings + update plan, without requiring a separate top-level workflow run.
Invocation:
  Signature: Workflow("WebOS_Engine_JS-Documentation", "WebOS.Cache", "Human language requirement")
  TargetsMeaning: >
...
## 2) Integration
- How WebOS loads/uses this engine (high-level)
...
- 2-4 practical examples:
  ...
  - Runtime usage example (how app/module would call WebOS engine)
Rules:
