...
Invocation:
  - Workflow("WebOS_Engine_PHP", "WebOS.Troubleshoot", "What you want in this engine (required)")
  - Workflow("WebOS_Engine_PHP", "WebOS.Troubleshoot, WebOS.Cache, WebOS.Auth, WebOS.Account")
  - Workflow("WebOS_Engine_PHP", "WebOS.Cache, WebOS.Auth", "Aese kr do wese kr do")
TargetsSyntax:
  - Comma-separated list of engine names (without extension)
  - Example: "WebOS.Cache, WebOS.Auth"
Outputs:
  Planning:
    Path: "/Aliens/.Alien/{Developer_Username}/Planning/WebOS/Engine/PHP/"
    Pattern: "{EngineName}.plan.md"
  Code:
    Path: "/Aliens/WebOS/{Version}/Engine/"  # Version resolves to latest PHP.* (fallback PHP.26.00.00)
    Pattern: "{EngineName}.php"
  Documentation:
    Path: "/Aliens/Docs/WebOS/Engine/PHP/"
...
- Extension kabhi include mat karo (".php" forbidden in targets).

Normalization (WebOS Engine convenience):
- Agar item `WebOS.` se start nahi hota (e.g., `Cache`) to normalize: `WebOS.{Item}`.

Notes:
- `WebOS.Troubleshoot` is a normal engine target name (not a special mode/keyword).
...
- "WebOS.Cache" valid; "WebOS Cache" invalid unless workflow explicitly supports aliasing.
...
Rule:
- If the resolved engine file already exists => run `WebOS_Engine_PHP_Update` for that target.
- If it does not exist => run the normal create flow: Planning → Code → Documentation.

Why:
- User requirement: running `WebOS_Engine_PHP` should automatically switch to update-mode when the target file already exists.

Implementation Notes (spec-level):
- Resolve `{Version}` using ACC.Paths policy (latest; fallback `PHP.26.00.00`).
- Existence check path:
  - `/Aliens/WebOS/{Version}/Engine/{EngineName}.php`
- When update-mode triggers:
  - Call: `Workflow('WebOS_Engine_PHP_Update','{EngineName}','{Description}')`
  - Then continue to next target (do NOT re-run Planning/Code/Docs in this parent workflow).

### Step 1: Planning
Call:
- Workflow("WebOS_Engine_PHP-Planning", "{EngineName}", "{Description}")

Expected Output:
- `/Aliens/.Alien/{Developer_Username}/Planning/WebOS/Engine/PHP/{EngineName}.plan.md`
...
### Step 2: Code
Call:
- Workflow("WebOS_Engine_PHP-Code", "{EngineName}", "{Description}")
...
Expected Output:
- `/Aliens/Docs/WebOS/Engine/PHP/{EngineName}.md`
...
- Code output MUST include:
  ...
  - logging markers consistent with existing WebOS engines
...
- Enterprise CodeHeader including explicit **Dependencies** list (real WebOS engines used)
- CodeFooter
...
- Engine must use existing WebOS ecosystem (e.g., `WebOS::Log()`, `WebOS::Security*()`, `WebOS::Policy*()`, `WebOS::Cache()` where relevant).
...
> New capability: `WebOS_Engine_PHP` ab CSV task list se bhi run ho sakta hai.
> Existing single-target / comma-target rules same rahenge; yeh rule **additional** hai.

## [10.1] Invocation
Allowed:
- `Workflow('WebOS_Engine_PHP','WebOS.Security')`  *(normal mode)*
- `Wrokflow('WebOS_Engine_PHP','{{FILE_NAME}}.csv')` *(CSV mode; legacy typo supported)*
...
- Trim
- If ends with `.php` or `.js` → strip extension (task files can be file-like)
- If does not start with `WebOS.` → prefix `WebOS.`
...
- If target engine already exists => run `WebOS_Engine_PHP_Update` and mark stages.
...
If exists:
- Run: `Workflow('WebOS_Engine_PHP_Update','{EngineName}','{RowDescription}')`
...
- If `planning != 1` then run:
  - `Workflow('WebOS_Engine_PHP-Planning', '{EngineName}', '{RowDescription}')`
- On success: set `planning=1`
- On fail: set `planning=2`, set `status=2`, skip remaining stages for that row.

### Stage B: Code
- If `code != 1` then run:
  - `Workflow('WebOS_Engine_PHP-Code', '{EngineName}', '{RowDescription}')`
...
- If `code_save != 1` then verify:
  - engine file exists at `/Aliens/WebOS/{Version}/Engine/{EngineName}.php`
  - `php -l` passes
...
### Stage D: Docs
- If `doc != 1` then run:
  - `Workflow('WebOS_Engine_PHP-Documentation', '{EngineName}', '{RowDescription}')`
