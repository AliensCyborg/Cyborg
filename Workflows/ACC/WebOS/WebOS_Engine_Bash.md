---
# [ACC.WORKFLOW] WebOS Engine Bash (Orchestrator)
WorkflowId: WebOS_Engine_Bash
WorkflowName: WebOS Engine Bash
...
Invocation:
- Workflow("WebOS_Engine_Bash", "WebOS.Cache, WebOS.Auth", "Aese kr do wese kr do")
...
## Step A: Planning
Call:
- Workflow("WebOS_Engine_Bash-Planning", "{EngineName}", "{Description}")
...
## Step B: Code
Call:
- Workflow("WebOS_Engine_Bash-Code", "{EngineName}", "{Description}")
...
2) Validate:
   - `Permissions.CanEditProdCode` must be true (writing WebOS engine files)
...
- `/Aliens/.Alien/{Developer_Username}/Task/Logs/ACC/WebOS_Engine_Bash/{RunId}.log.md`
...
CSV mode mirrors the standard WebOS CSV rules (Planning → Code → Documentation).

## [11.1] Invocation
Allowed:
- `Workflow('WebOS_Engine_Bash','WebOS.Cache')`
- `Workflow('WebOS_Engine_Bash','{FILE_NAME}.csv')`
