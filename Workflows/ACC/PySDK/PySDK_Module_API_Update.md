---
WorkflowId: PySDK_Module_API_Update
...
Summary: >
  Plural UPDATE-only workflow that updates one or multiple PySDK Module APIs based on Targets.
...
Calls:
  - "Workflows/ACC/PySDK/PySDK_Module_API_Update-Audit.md"
  - "Workflows/ACC/PySDK/PySDK_Module_API_Update-Planning.md"
  - "Workflows/ACC/PySDK/PySDK_Module_API_Update-Code.md"
  - "Workflows/ACC/PySDK/PySDK_Module_API_Update-Doc.md"
Inputs:
  - Name: Targets
    Type: string
    Required: true
...
## Purpose
`PySDK_Module_API_Update` ek **Plural** workflow hai jo 1 ya multiple **existing** PySDK Module APIs ko update karwata hai.

Example:
- `Workflow("PySDK_Module_API_Update", "User/UserNew, Account/AccountUpdate", "Existing APIs me validation improve karo")`

Iska matlab:
- 2 targets => same workflow **2 baar** run hoga (per target item).
- Har target item ke liye yeh 4 child workflows call karega:
  1) `PySDK_Module_API_Update-Audit`
  2) `PySDK_Module_API_Update-Planning`
  3) `PySDK_Module_API_Update-Code`
  4) `PySDK_Module_API_Update-Doc`
...
**Important:** Actual type (API) is decided by this workflow id (`PySDK_Module_API_Update`).
...
Calls:
1) `Workflow("PySDK_Module_API_Update-Audit", "<ModuleName/ApiName>", "<Description>")`
1) `Workflow("PySDK_Module_API_Update-Planning", "<ModuleName/ApiName>", "<Description>")`
2) `Workflow("PySDK_Module_API_Update-Code", "<ModuleName/ApiName>", "<Description>")`
3) `Workflow("PySDK_Module_API_Update-Doc", "<ModuleName/ApiName>", "<Description>")`
...
1) **Module Class Update**
   - Required when any class method is added/modified for the API:
     - `PySDK_Module_Class_Update` MUST be called for that module.

2) **Module SQL Update (Schema change required)**
   - If new table/columns required:
     - MUST call `PySDK_Module_SQL_Update`.
   - Note:
     - `PySDK_Module_SQL_Update` is available and should be used instead of blocking.
