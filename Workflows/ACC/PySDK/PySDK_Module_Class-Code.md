---
WorkflowId: PySDK_Module_Class-Code
WorkflowName: PySDK Module Class Generator (Code)
Type: Workflow_Singular-Code
ParentWorkflow: PySDK_Module_Class
Domain: PySDK
Engine: ACC.Workflow
Status: Draft
Version: 26.00.00
...
Outputs:
  - Code: "/Aliens/PySDK/Python.{Latest}/Module/{Module}/py/index.py"
...
## [0] Purpose
Is workflow ka kaam: **PySDK Module ke liye class file (index.py) generate/update** karna.

- Target example:
  - `Workflow("PySDK_Module_Class", "User, Account", "CRUD + business logic clean and consistent")`
- Output example:
  - `/Aliens/PySDK/Py.26.00.00/Module/User/py/index.py`
...
Is workflow me unique rules **sirf PySDK Module Class** ke context ke liye rahenge.
...
- Means create `/py/{FileBase}.py` and wire into `Python/index.py`.
...
- Deterministic rules (same as PySDK reference):
...
### Version
- Default: **latest**
- Fallback: `Py.26.00.00` (only if auto-detect not possible)

### Paths (must match Aliens ecosystem)
- PySDK root:
  - `/Aliens/PySDK/Python.{Version}/Module/`

For each module `{M}`:
- Module root:
  - `/Aliens/PySDK/Python.{Version}/Module/{M}/`
- Class file output (main):
  - `/Aliens/PySDK/Python.{Version}/Module/{M}/py/index.py`

For file-scope `{M}/{FileBase}`:
- Part file output:
  - `/Aliens/PySDK/Python.{Version}/Module/{M}/py/{FileBase}.py`
- Wire-up required in `Python/index.py` (require + constructor wiring).
...
- If `/Module/{M}/` missing => **error**
  - Guidance in error: “Run `PySDK_Module` or `PySDK_Module_Structure` first.”

- If `/Module/{M}/py/` missing => **error**
  - Guidance: “Run `PySDK_Module_Structure` first.”

NoDelete policy:
- Kisi bhi existing file/folder ko delete nahi karna.
- Create missing file allowed (index.py), but missing parent folders => error (because structure workflow owns it).
...
Rule:
- Examples source:
  - `/Aliens/PySDK/Python.{Version}/Module/*/py/index.py`
- Pick 4 best matches:
  - 2 singular modules (single-table surface)
  - 2 plural modules (modules that clearly have `{ModuleName}_*.py` parts and/or multiple `{ModuleName}_*` tables) if available
...
### 6.1 File contract
Output file: `/Module/{M}/py/index.py`
...
- Main entrypoint (backward compatible):
  - `/Aliens/PySDK/Python.{Version}/Module/{ModuleName}/py/index.py`
- Facade traits:
  - `/Aliens/PySDK/Python.{Version}/Module/{ModuleName}/py/_Trait.py`
- Part classes:
  - `/Aliens/PySDK/Python.{Version}/Module/{ModuleName}/py/{ModuleName}_{Part}.py`

**Main class rules (index.py):**
- MUST `require_once` the part files and `_Trait.py` using `__DIR__` before `class {ModuleName}`.
...
- MUST keep `$Python['{Module}']['function'][]` complete for all public methods (including facade wrappers).

**Facade traits rules (_Trait.py):**
...
**Part classes rules ({ModuleName}_{Part}.py):**
...
- When target is `Module/FileBase`, generator MUST:
  - Create `{FileBase}.py` with class `{FileBase}`.
  - Ensure `index.py` requires the part file and constructs the part instance.
...
8. Code footer (mandatory):
  ...
  - Footer must include:
    ...
    - Usage Guide (how to call the module via PyOS/router)
...
- `ERR_PHP_FOLDER_MISSING`: Python/ folder missing
...
- `index.py` created/updated successfully
