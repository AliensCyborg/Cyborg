---
WorkflowId: PySDK_Module_Class-Documentation
...
**Purpose (Hinglish):**  
Yeh workflow, PySDK ke kisi Module (e.g., `User`, `Account`) ki **main class** (usually `Python/index.py`) ki **documentation** generate karta hai — targets aur description ke hisaab se — aur docs ko `/Aliens/Docs/PySDK/` me save karta hai.

> Note: Yeh workflow *sirf docs* banata hai. Code generate/modify ka kaam `PySDK_Module_Class-Code` me hota hai.
...
**Meaning:**
- `User` = Singular module when only required table is `User` (primary entry class `Python/index.py`).
...
- `/Aliens/PySDK/Py.26.00.00/Module/{ModuleName}/`

If auto-latest decide possible nahi:
- Default fallback: `Py.26.00.00`

### Step B: Resolve main class file
Primary class file (95% cases):
- `/Aliens/PySDK/Py.26.00.00/Module/{ModuleName}/py/index.py`
...
- SQL structure: `/Aliens/PySDK/Py.26.00.00/Module/{ModuleName}/sql/index.sql`
- API folder: `/Aliens/PySDK/Py.26.00.00/Module/{ModuleName}/api/`
- Cron folder: `/Aliens/PySDK/Py.26.00.00/Module/{ModuleName}/cron/`
- Webhook folder: `/Aliens/PySDK/Py.26.00.00/Module/{ModuleName}/webhook/`
...
- `/Aliens/Docs/PySDK/Module/{ModuleName}/Class/{Topic}.md`
...
1) **File Map (Python/)**
   - `Python/index.py` (main class entrypoint)
   - `Python/_Trait.py` (facade traits)
   - `Python/{ModuleName}_{Part}.py` (partition classes)
...
4) **How to add a new partition**
   - Create `{ModuleName}_{Part}.py`
   - Add trait `{ModuleName}_{Part}Facade` in `_Trait.py`
   - Wire constructor injection + default init in `index.py`
...
## [10] Examples
### Example 1
`Workflow("PySDK_Module_Class-Documentation", "User", "Explain class, methods list, 5 usage examples")`

### Example 2
`Workflow("PySDK_Module_Class-Documentation", "User, Account", "Plural (multi-table) mapping clearly mention karna")`
