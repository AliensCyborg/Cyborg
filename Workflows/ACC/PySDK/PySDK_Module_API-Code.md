---
WorkflowId: PySDK_Module_API-Code
WorkflowName: PySDK Module API Code Generator
...
**Role (Hinglish):**  
Yeh file `PySDK_Module_API` workflow ke liye **API code generator** hai.  
Iska kaam: planning ko source-of-truth maan kar **API Python file create/update** karna, proper folder me save karna, aur run-record me exact changes + quality summary store karna.

> Important: Is file me sirf **PySDK API generation** ke unique rules honge.  
...
- `Module/ApiName` => PySDK module + API file name (without extension)
...
For this workflow:
- Module = PySDK Module folder name under `/Aliens/PySDK/.../Module/`
- ApiName = Python file name under `{Module}/api/`
...
## [02] Resolve paths (unique mapping for PySDK APIs)
Using `ACC.Paths.md` resolver contract:

### PySDK root (version)
- Primary: latest available `Python.{Latest}`
...
- Generator MUST:
  ...
  - assume module class may use modular layout (`index.py` + `_Trait.py` + `{Module}_{Part}.py`) but primary entry remains `Python/index.py`
...
Generator MUST produce an enterprise-grade Python API file that includes:
...
5) **Database safety**:
   - No raw unsafe SQL
   - Use PyOS DB engine patterns (from `PyOS_Engines.md` / `PyOS_System.md`)

### 5.1) Hard Policy (Aliens Company) — API files are glue only
For **every** PySDK Module API file under:
...
- Add/modify a **method** in `{ModuleClass}` (`/py/index.py`) inside the **existing** module class.
...
8) **Logging** (very important):
   - Since this is PySDK (SDK/App layer), use **d()** logs, NOT x()
...
- Never document or encourage running:
  - `/Aliens/PySDK/.../Module/<Module>/api/<ApiName>.py?...`
...
### B) Response envelope (PySDK base)
...
- Routing/verification is handled by PyOS/Base router layer.
...
1) Same module, same folder:
   - Search `{ModuleRoot}/api/` for any existing `.py` files
   - Pick up to 2 most recently modified

2) Similar modules (fallback):
   - Search `/Aliens/PySDK/{Python.Version}/Module/*/api/*.py`
...
For normal PySDK API create/update, usually:
...
- Ensure required doc signals are present in run-record so that:
  - `PySDK_Module_API-Documentation.md` can generate docs under `/Aliens/Docs/PySDK/...`
