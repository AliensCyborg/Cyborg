---
WorkflowId: PySDK_Module_Class_Update-Planning
WorkflowName: PySDK Module Class Update Planning
...
Outputs:
  PlanningFile:
    Location: "/Aliens/.Alien/{Developer_Username}/Planning/PySDK/Module/{ModuleName}/Class.Update.md"
    SaveStrategy: "create-or-update"
  AuditFile:
    Location: "/Aliens/.Alien/{Developer_Username}/Planning/PySDK/Module/{ModuleName}/Class.Update.audit.md"
...
- If target is `ModuleName/FileBase`, planning MUST be saved as:
  - `/Aliens/.Alien/{Developer_Username}/Planning/PySDK/Module/{ModuleName}/Class.Update.{FileBase}.md`
- And audit input MUST be read from:
  - `/Aliens/.Alien/{Developer_Username}/Planning/PySDK/Module/{ModuleName}/Class.Update.{FileBase}.audit.md`
Safety:
  NoDelete: true
  ModifyOnly: true
...
# PySDK_Module_Class_Update-Planning.md

## [00] Purpose
Yeh workflow **sirf planning** generate karta hai, update scenario ke liye:
`Workflow("PySDK_Module_Class_Update", "User, Account", "...")`

Core goal:
- Existing module class (`Python/index.py`) ko read + analyze karke
- Minimal, safe, requirement-focused update plan banana
- Plan ko save karna in developer workspace:
  `/Aliens/.Alien/{Developer_Username}/Planning/PySDK/Module/{ModuleName}/Class.Update.md`
...
- System refs: `PySDK.md`, `PyOS_Engines.md`, `PyOS_System.md`
...
- If target is `ModuleName`:
  - `/Aliens/PySDK/{LatestVersion}/Module/{ModuleName}/py/` MUST exist and MUST contain at least one `.py` file.
  - `Python/index.py` is still the primary anchor file for module-wide planning.
- If target is `ModuleName/FileBase`:
  - `/Aliens/PySDK/{LatestVersion}/Module/{ModuleName}/py/{FileBase}.py` MUST exist.
...
- If part unit evidence exists (`{ModuleName}_*`) but base `{ModuleName}` is missing:
  ...
  - Include fix instruction: “Add/restore base table `{ModuleName}` via `PySDK_Module_SQL_Update` before applying code updates.”

## [06] Existing Class Analysis (Index.py)
Planning MUST do:
- If target is `ModuleName`: read and analyze the full `Python/index.py` first, then enumerate other Python/*.py files for scope.
- If target is `ModuleName/FileBase`: read and analyze only `/py/{FileBase}.py` (plus `Python/index.py` only if required for context; do not expand scope).
...
### 6.1 Canonical PySDK Module Skeleton Checks (based on reference modules)
...
6) `$Python['{Module}']` metadata + `$Python['{Module}']['function'][]` manifest present (top-level)
7) Exactly one primary `class {ModuleName}` defined in `Python/index.py` for the module.
  - If the module is `PluralModule`, part classes (`{ModuleName}_{Part}`) MUST live in separate files under `Python/` and be loaded via `require_once` in `index.py` (not defined inline inside `index.py`).
...
- Deterministic audit artifact MUST exist before planning:
  - Module target: `/Aliens/.Alien/{Developer_Username}/Planning/PySDK/Module/{ModuleName}/Class.Update.audit.md`
  - File target: `/Aliens/.Alien/{Developer_Username}/Planning/PySDK/Module/{ModuleName}/Class.Update.{FileBase}.audit.md`
...
- Main: `/Aliens/PySDK/{LatestVersion}/Module/{ModuleName}/py/index.py`
- Traits: `/Aliens/PySDK/{LatestVersion}/Module/{ModuleName}/py/_Trait.py`
- Part files: `/Aliens/PySDK/{LatestVersion}/Module/{ModuleName}/py/{ModuleName}_{Part}.py`
...
- Create `_Trait.py` if missing.
- Add `require_once __DIR__ . '/{ModuleName}_{Part}.py';` lines.
- Add `require_once __DIR__ . '/_Trait.py';`.
...
1) **CodeHeader + Version Footprint**
  - Ensure `# Copyright (c) A|iens. All rights reserved.` exists at top (line 2) for Python files.
  - Ensure header block includes: `AliensStyle`, `LastModified (UTC Z)`, `PySDK`, `PyOS`, `App`.
...
10) **Syntax/Lint Gate**
  - Plan MUST include a `python -m py_compile` verification step after each patch batch.
