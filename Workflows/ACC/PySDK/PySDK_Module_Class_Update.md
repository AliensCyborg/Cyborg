---
WorkflowId: PySDK_Module_Class_Update
WorkflowName: PySDK Module Class Update
Type: Workflow_Plural
Domain: PySDK
Category: Module
Intent: "Update existing PySDK module Python classes (Python/*.py) for one or multiple modules (NO auto-create)"
Status: Draft
Version: 26.00.00
...
Calls:
  ...
  - "PySDK_Module_Class_Update-Planning"
  - "PySDK_Module_Class_Update-Code"
  - "PySDK_Module_Class_Update-Documentation"
...
## [00] Purpose
Yeh workflow **sirf UPDATE** ke liye hai: PySDK ke module ki primary class file (`Python/index.py`) ko improve/modify karna.
...
Example:
`Workflow("PySDK_Module_Class_Update", "User, Account, Web3", "Class ko refactor + secure + performance improve")`
...
- `W3` -> ModuleName (update whole module Python folder)
...
- Target item is either:
  - `ModuleName`
  - `ModuleName/FileBase`
    - `FileBase` MUST be a Python filename base WITHOUT extension (example: `W3_Account`, not `W3_Account.py`).
    - `FileBase` MUST NOT contain additional slashes, dots, or traversal sequences.
- ModuleName should match folder name under: `/Aliens/PySDK/{LatestVersion}/Module/{ModuleName}/`
...
- `Workflow('PySDK_Module_Class_Update', 'W3', '...')` means update **ALL** Python class files directly under:
  - `/Aliens/PySDK/{LatestVersion}/Module/W3/py/*.py`
  - This includes: `index.py`, `_Trait.py`, and any `W3_*.py` part classes and any other `.py` class files in that folder.
- `Workflow('PySDK_Module_Class_Update', 'W3/W3_Account', '...')` means update **ONLY**:
  - `/Aliens/PySDK/{LatestVersion}/Module/W3/py/W3_Account.py`

## [04] Execution Plan (Plural Orchestration)
Per Target, sequence:
0) `PySDK_Module_Class_Update-Audit` (read-only audit)
1) `PySDK_Module_Class_Update-Planning` (plan create)
2) `PySDK_Module_Class_Update-Code` (apply update to code)
3) `PySDK_Module_Class_Update-Documentation` (update docs)
...
Child workflows MUST enforce that updates do not break canonical module skeleton ordering (declare/Access/header/bootstrap/imports/$Python manifest/class).

## [07] Outputs
- Planning saved to:
  `/Aliens/.Alien/{Developer_Username}/Planning/PySDK/Module/{ModuleName}/Class.Update.md`
- Code updated in place:
  `/Aliens/PySDK/{LatestVersion}/Module/{ModuleName}/py/index.py`
...
- Docs (optional, if docs step runs):
  - `/Aliens/Docs/PySDK/Module/{ModuleName}/Class.{FileBase}.md`
...
`Workflow("PySDK_Module_Class_Update", "User", "Existing class ko optimize karo, auth checks add karo")`

2) Plural module update (multi-table + units):
`Workflow("PySDK_Module_Class_Update", "User", "Plural module hai (User + User_Address + User_Meta). Tables mapping + modularization update karo")`

3) Multi-module update:
`Workflow("PySDK_Module_Class_Update", "User, Account, Web3", "Security hardening + docs refresh")`

---

## [11] Execution Protocol Upgrade (Required)
Jab bhi aap `PySDK_Module_Class_Update` run karte ho, iska matlab yeh hai:

### 11.1 Planning-first (Non-negotiable)
- Agent MUST first read the full target file and generate a checklist-style plan (see `PySDK_Module_Class_Update-Planning`).
...
Example:
`Done: English-only logs/comments (Web3 index.py). python -m py_compile OK. Next: SECTION dividers. Ok?`
